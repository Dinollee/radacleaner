#!/usr/bin/env python3
"""
KPI v5: добавлен Bill Quality Score.

Формула (0-100):
  Score = 0.25×norm(LEI) + 0.15×norm(ПЯ) + 0.10×norm(ПДА) + 0.05×norm(ВКП) 
        + 0.10×norm(Конверсія) + 0.15×norm(Комітет) + 0.20×norm(BillQuality)

Bill Quality Score = avg((significance + impact) / 2) по проанализированным законам
Диапазон: 0-5 (significance и impact от 1 до 5)
"""
import psycopg2
import os
...
def calc_kpi_v5():
    ...
    for row in cur.fetchall():
        dep_id, name, faction, lei, py, pda, vkp, total_bills, total_laws, committee_score, quality = row
    ...
    for faction, indices in factions.items():
        ...
        qual_faction[faction] = normalize([deputies[i]["quality"] for i in indices])
...
if __name__ == "__main__":
    print("Розрахунок KPI v5 (з якістю законів)...")
    deputies = calc_kpi_v5()
    print(f"Оброблено {len(deputies)} депутатів")

    save_kpi_v5(deputies)
    print("KPI v5 збережено в БД")

    # Top 15
    top = sorted(deputies, key=lambda x: x["score"], reverse=True)[:15]
    print("\nТоп-15 за KPI Score v5:")
    print(f"  {'Name':<25} {'Faction':<20} {'Score':<8} {'LEI':<8} {'Quality':<8} {'ПЯ':<8} {'ПДА':<8} {'Comm':<8}")
    print("-" * 110)
    for d in top:
        print(f"  {d['name']:<25} {d['faction']:<20} {d['score']:<8.1f} {d['lei']:<8.1f} {d['quality']:<8.2f} {d['py']:<8.1f} {d['pda']:<8.1f} {d['committee_score']:<8.1f}")
