...
## Level C — Реєстр лобіювання НАЗК ВНЕДРЕНО (2026-08-23)

**Джерело розкрите**: transparency.nazk.gov.ua — SPA, але публічне API без auth.
Карта ендпоинтів (витягнута з JS-бандла, контролер /api/v1/public):
allsubjects, subject?id={guid}, reports?subjectId={guid}, report?id={reportGuid}
(⚠️ подвійне JSON-кодування відповідей — unwrap), find (POST searchString),
searchFiltersDictionaries, dashboards. Захват через headless-браузер + grep бандла.

**Обсяг на 08.2026**: 205 суб'єктів лобіювання (усі активні), 971 об'єкт
лобіювання у деклараціях, 188 зв'язків із законопроєктами ВРУ (валідація
EXISTS bills.bill_number). Топ: AmCham (243), Німецько-Українська ТПП (111),
Всеукраїнська аграрна рада (98), Асоціація газовидобувних компаній (78).

**Інфраструктура**: lobbying_subjects/lobbying_objects (migration 028),
sync_lobbying_registry.py щодня 07:00 (~8 хв, DELAY 0.4с), stats_cache
`lobbying_registry_meta`. Регекс витягу номерів: №\s*(\d{3,6}) — гола латинська
N прибрана (давала хибні спрацювання на рішеннях міськрад). API /api/lobbying
(?bill=NNNN по закону; ?limit= стрічка + топ-суб'єкти). UI: вкладка «🏛 Лобіювання»
в картці закону + блок на вкладці Клуби.

**Юридична рамка**: це ДЕКЛАРАЦІЇ самих лобістів (закон зобов'язує їх
декларувати контакти) — проєкт лише агрегує офіційні факти, без власних
вердиктів. Дисклеймери в UI.

**Далі**: моніторити наповнення реєстру (реєстр молодий); потенційно — зв'язок
«лобіст ↔ депутат» коли НАЗК почне публікувати контакти з народними депутатами
у структурованому вигляді.

## Level C — Декларації НАЗК, фаза 1 ВНЕДРЕНО (2026-08-23)

**Розвідка**: новий реєстр public.nazk.gov.ua. Знайдено ЖИВИЙ v2 JSON API
(public-api.nazk.gov.ua/v2/documents/{uuid}) — лінк на нього схований у меню
«Інші формати» списку декларацій. Пошук: GET /documents/list?q={прізвище}
(серверний HTML; ініціали в запиті ламають пошук). У рядку списку: ПІБ, посада,
дата подання, uuid → префільтр БЕЗ завантаження карток.

**Пайплайн** (sync_nazk_declarations.py): прізвище депутата → список → фільтр
посада «народний депутат» (case-insensitive) + збіг ПІБ (прізвище + перша
літера імені) → найсвіжіша за датою подання → v2 JSON → data.step_8
(корпоративні права: name, ЄДРПОУ, legalForm, частка %) → deputy_declarations
(migration 030). КВЕД у декларації НЕМАЄ — галузь компанії = наступна фаза
(відкритий реєстр ЄДР по ЄДРПОУ або класифікація назви з чесним маркером).

**Готчі**: подвійне кодування тут НЕ зустрілось (на відміну від Реєстру
прозорості), але регекс рядків списку з optional-групами + lazy `.*?`
мовчки пропускав дати → переписано на поблочний розбір між <article>.
Таймер nazk-declarations щодня 06:00.

**Наступна фаза**: КВЕД по ЄДРПОУ → авто-перетин «компанія депутата × галузі
його ж законів (interest_sectors)»; блок «Бізнес за декларацією» уже показує
факти з дисклеймером «дані самих депутатів».

## Session 2026-08-23 — «Группы интересов» полный цикл за один день

Пользовательский вопрос утром: «можем ли отслеживать голосования по группам
интересов? кто что и кому лобирует?» → трёхуровневый план → все уровни shipped:

1. **Level A** (79aac0e): Клубы голосования — numpy-матрица согласия на спорных
   голосованиях, voting_allies, вкладка «🤝 Клуби».
2. **Level B** (8da82d1): interest_sectors в LLM-анализе законов (словарь 16
   галузей) + профіль інтересів депутата + бекфілл 300/день.
3. **Level C фаза 1** (7fc5209): Реєстр прозорості НАЗК — открытый API раскрыт,
   205 лоббистов / 971 объект / 188 связей с законами ВРУ; вкладка «🏛 Лобіювання».
4. **Портреты + декларации** (9537824): LLM-портрет депутата из наших данных
   (замена шаблонных сигналов) + скрапер деклараций НАЗК через найденный v2 JSON
   API (276/389 депутатов, 105 с компаниями).
5. **C фаза 2** (0952b53): LLM-классификация компаний по отраслям (180/301,
   auto_name маркер) + баннер «⚡ Перетин» бизнес∩законы в карточке депутата.

Попутно: фикс календаря (изменения считаются по официальной pass_date, а не по
моменту обнаружения синком — репорт пользователя про №15360), watchlist +
@gnapinfo + @yzheleznyak (8b13f78), прод-деплой stale alias починен прямым
wrangler deploy --branch main.

Открытые хвосты для следующей сессии:
- Портреты: ✅ ЗАКРЫТО 2026-09-07. Аудит показал 387/387 активных депутатов
  с портретом. 71 без портрета = все с end_date!=NULL (звільнені), by design.
  Предыдущая запись («198+/389») была с 2026-08-23 и устарела.
- Бекфілл interest_sectors йде ~300/день (~13.6 днів залишилось, 39.2% ready).
- Верифікація votesync при відновленні пленарних засідань ВР (вересень).
- eu_news_* автоочистка stats_cache: ✅ ЗАКРЫТО 2026-09-07. Аудит: 0 ключів
  по патернах eu_news/harmonization/pulse/directive/active_mps. stats_cache
  total = 265. Чищення B4 (437→110, див. рядок 2154) вже покрило.
- Далекий горизонт: КВЕД по ЄДРПОУ з відкритого реєстру (точний, замість
  auto_name класифікації назв).
- sync_eu_tracker.timer: ✅ ЗАКРЫТО 2026-09-07. enabled + active (09:00 daily).
  Запис у NEXT STEPS «OPEN» застарів.

## Session 2026-08-25: citizen_impact — «що зміниться для громадянина»

### Decision: окремий виклик з ПОВНИМ текстом, НЕ поле у FINAL_PROMPT
- FINAL_PROMPT бачить лише summary чанків (1-2 речення/чанк) — пари «до/після»
  втратили б конкретику (суми/строки). Пілот, формулировки якого схвалено,
  їв повний plain_text (50K символів — контекст nemotron 1M дозволяє один виклик).
- Один продюсер = backfill_citizen_impact.py (щодня 05:30, 300/ніч,
  risk_score DESC) покриває І історію (~6.4K непроцедурних), І нові аналізи —
  однакова якість, нуль впливу на пропускну здатність night-batch.
- Процедурним і законам з текстом <1200 символів — фіксований JSON без LLM
  (3209 позначено миттєво).
- Антигалюцинації: правила промпта + мовний чекер (_is_ukrainian: латиниця >30%
  або змішані слова типу «wprowadжує» → retry до 3 спроб).
- API без змін: /api/bills/:id повертає SELECT * (json_data їде сам).
- Пілот: scripts/pilot_citizen_impact.py (5 законів, формалянки схвалено).
