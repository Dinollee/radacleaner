...
- Портреты: фоновой прогон шёл после таймаута systemd (198+/389); проверить,
  что к утру все 389 закрыты (таймер 04:30 дотягивает сам).
- Бекфілл interest_sectors йде ~300/день (~22 дні) — перетини «⚡» густітимуть.
- Верифікація votesync при відновленні пленарних засідань ВР (вересень).
- eu_news_* автоочистка stats_cache.
